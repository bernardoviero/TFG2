...
+!obterDados <- 
    ...
    .wait(4000);
    !enviar_tempo_chegada_ao_controlador;
    !aguardar_tempo_chegada.

+!enviar_tempo_chegada_ao_controlador <-
    ?id(Id);
    ?tempo_chegada(TempoChegada);
    .send(controller, tell, tempo_chegada(Id, TempoChegada));
    .println("Enviado tempo de chegada ao controlador: ", TempoChegada).
...
+!kqml_received(Sender, ILF, Content, MsgID) <- 
    .println("Mensagem recebida de ", Sender, " com ILF ", ILF, " e conteudo ", Content).
